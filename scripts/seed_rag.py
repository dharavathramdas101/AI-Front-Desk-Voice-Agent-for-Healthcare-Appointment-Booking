...
def chunk_markdown(text: str, source: str, max_tokens: int = 150) -> list[Chunk]:
    ...
    chunks, buffer, buffer_len, chunk_idx = [], [], 0, 0
...
def seed() -> None:
    ...
    for md_file in sorted(config.KB_PATH.glob("*.md")):
        chunks = chunk_markdown(md_file.read_text(encoding="utf-8"), source=md_file.name)
        all_chunks.extend(chunks)
        print(f"  {md_file.name}: {len(chunks)} chunks")

    print(f"\nIndexing {len(all_chunks)} total chunks...")
    retriever.index_documents(all_chunks)
    print(f"Done. Seeded {len(all_chunks)} chunks into ChromaDB.")
